...
• Flow: User asks → Backend retrieves relevant docs from ChromaDB → Augments prompt → Sends to LLM → Streams response back to user with source citations

• Result: Accurate, verifiable, domain-specific answers grounded in company data

• Deploy everything local in one repo (no Docker)
